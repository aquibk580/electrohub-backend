...
export async function getSalesStatistics(req, res) {
    try {
        ...
        const sales = products.reduce((acc, product) => acc +
            product.orderItems.length *
                product.price *
                (1 - (product.offerPercentage ? product.offerPercentage / 100 : 0)), 0);
        const last7DaysSales = await Promise.all([...Array(7)].map(async (_, i) => {
...
export async function getOrdersData(req, res) {
    try {
